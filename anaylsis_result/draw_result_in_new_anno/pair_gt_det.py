def get_result_pair_info():
    result_pair = "/home/user/PycharmProjects/caltech_new_anno/out/result_pair1.txt"

    rf = open(result_pair, "r")
    file_name = []
    contx = rf.readline()


    while contx:
        file_name.append(contx.split('-')[0])
        contx = rf.readline()

    return file_name
    pass

print  get_result_pair_info()
...
def generate_result(resource_path, des_path):
    ...
    cnt = 0
    tmp_dict = {}
    result_pair_list = get_result_pair_info()
    while content:
        #print content
