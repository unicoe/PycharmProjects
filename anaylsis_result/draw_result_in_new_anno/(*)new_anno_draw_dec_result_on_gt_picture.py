...
def draw_bbox(tmp_dict, tmp_dict1):
    ...
    cur_path = "/home/user/draw_train_and_test/test"
    dirList = []
    fileList = []
    files = os.listdir(cur_path)
                ...
                if inDict_name in tmp_dict and inDict_name in tmp_dict1:
                    ...
                    for idx_bbox in bboxList:
                        ...
                        #idx_gt [375.544632086852,172.740631992242,22.3884197828709,49.0229191797346]
                            ...
                            # plt.imshow(im)
                            # plt.show()
                    mkdir('/home/user/draw_result/4_16/test/' + str(inDict[-3] + '/' + inDict[-2]))
                    if DRAW_IMG:
                        cv2.imwrite(
                                    '/home/user/draw_result/4_16/test/' + str(inDict[-3] + '/' + inDict[-2]) + '/' + bot + ".jpg",
                                    im)
                    elif WRITE_TXT:
                        wtxt.close()
...
dec_result = generate_result("/home/user/PycharmProjects/handle_result/result_detection/handle_6_11_nms_upper_person_all_det_test.txt")

draw_gt1andresult(dec_result)