...
class SolverWrapper(object):
	...
	# rfcn multi_branch
        scale_bbox_params_rfcn = (cfg.TRAIN.BBOX_REG and
                             cfg.TRAIN.BBOX_NORMALIZE_TARGETS and
                             net.params.has_key('local_cls_p3') and
                             net.params.has_key('local_bbox_p3'))
	...
	# coupleNet
        ...
        if scale_bbox_params_rfcn: # rfcn multi_branch
            # save original values
            orig_0_p3 = net.params['local_bbox_p3'][0].data.copy()
            orig_1_p3 = net.params['local_bbox_p3'][1].data.copy()
            repeat = orig_1_p3.shape[0] / self.bbox_means.shape[0]

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['local_bbox_p3'][0].data[...] = \
                    (net.params['local_bbox_p3'][0].data *
                     np.repeat(self.bbox_stds, repeat).reshape((orig_1_p3.shape[0], 1, 1, 1)))
            net.params['local_bbox_p3'][1].data[...] = \
                    (net.params['local_bbox_p3'][1].data *
                     np.repeat(self.bbox_stds, repeat) + np.repeat(self.bbox_means, repeat))

            # save original values
            orig_0_p3_couple = net.params['global_bbox_pred_p3'][0].data.copy()
            orig_1_p3_couple = net.params['global_bbox_pred_p3'][1].data.copy()
            repeat_1_p3_couple = orig_1_p3_couple.shape[0] / self.bbox_means.shape[0]

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['global_bbox_pred_p3'][0].data[...] = \
                (net.params['global_bbox_pred_p3'][0].data *
                 np.repeat(self.bbox_stds, repeat_1_p3_couple).reshape((orig_1_p3_couple.shape[0], 1, 1, 1)))
            net.params['global_bbox_pred_p3'][1].data[...] = \
                (net.params['global_bbox_pred_p3'][1].data *
                 np.repeat(self.bbox_stds, repeat_1_p3_couple) + np.repeat(self.bbox_means, repeat_1_p3_couple))



            # save original values
            orig_0_p2 = net.params['local_bbox_p2'][0].data.copy()
            orig_1_p2 = net.params['local_bbox_p2'][1].data.copy()
            repeat = orig_1_p2.shape[0] / self.bbox_means.shape[0]

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['local_bbox_p2'][0].data[...] = \
                    (net.params['local_bbox_p2'][0].data *
                     np.repeat(self.bbox_stds, repeat).reshape((orig_1_p2.shape[0], 1, 1, 1)))
            net.params['local_bbox_p2'][1].data[...] = \
                    (net.params['local_bbox_p2'][1].data *
                     np.repeat(self.bbox_stds, repeat) + np.repeat(self.bbox_means, repeat))

            # save original values
            orig_0_p2_couple = net.params['global_bbox_pred_p2'][0].data.copy()
            orig_1_p2_couple = net.params['global_bbox_pred_p2'][1].data.copy()
            repeat_1_p2_couple = orig_1_p2_couple.shape[0] / self.bbox_means.shape[0]

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['global_bbox_pred_p2'][0].data[...] = \
                (net.params['global_bbox_pred_p2'][0].data *
                 np.repeat(self.bbox_stds, repeat_1_p2_couple).reshape((orig_1_p2_couple.shape[0], 1, 1, 1)))
            net.params['global_bbox_pred_p2'][1].data[...] = \
                (net.params['global_bbox_pred_p2'][1].data *
                 np.repeat(self.bbox_stds, repeat_1_p2_couple) + np.repeat(self.bbox_means, repeat_1_p2_couple))
	    print 'aaaa'



        if scale_bbox_params_couplenet: # pass couplenet
            # save original values
        ...
        if scale_bbox_params_rfcn:
            # restore net to original state
            net.params['local_bbox_p3'][0].data[...] = orig_0_p3
            net.params['local_bbox_p3'][1].data[...] = orig_1_p3

            net.params['global_bbox_pred_p3'][0].data[...] = orig_0_p3_couple
            net.params['global_bbox_pred_p3'][1].data[...] = orig_1_p3_couple

            net.params['local_bbox_p2'][0].data[...] = orig_0_p2
            net.params['local_bbox_p2'][1].data[...] = orig_1_p2

            net.params['global_bbox_pred_p2'][0].data[...] = orig_0_p2_couple
            net.params['global_bbox_pred_p2'][1].data[...] = orig_1_p2_couple

        if scale_bbox_params_couplenet: # pass
            # restore net to original state
