...
def draw_bbox(tmp_dict):
    ...
    cur_path = "/home/user/Downloads/caltech_data_set/datasets/inria_test/images/set01/V000_copy"
    dirList = []
    fileList = []
    files = os.listdir(cur_path)
        ...
        if inDict_name in tmp_dict :
            ...
            for idx_bbox in bboxList:
                ...
                if DRAW_IMG and w >= 0 and h>=0:
                    ...
                    # plt.imshow(im)
                    # plt.show()
            mkdir('/home/user/Disk1.8T/draw_result/inria/11_15_1/' + str(inDict[-3] + '/' + inDict[-2]))
            if DRAW_IMG:
                cv2.imwrite(
                            '/home/user/Disk1.8T/draw_result/inria/11_15_1/' + str(inDict[-3] + '/' + inDict[-2]) + '/' + bot + ".jpg",
                            im)
            elif WRITE_TXT:
                wtxt.close()
...
file_path = "/home/user/PycharmProjects/some_learn/Data_Set_handle/INRIA-Dataset/det_result/det/2018_11_15_Thu_07_48_54_det_test_person.txt"
