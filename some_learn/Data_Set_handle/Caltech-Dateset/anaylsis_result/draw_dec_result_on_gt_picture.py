...
def generate_result(resource_path, tmp_dict1):
    """
    :param path: 
    :return: 
    """
...
def generate_all_result(path):
    ...
    tmp1 = {}
    for fl in fileList:
        tmp1 = dict(tmp1, **generate_gt_dict(path + fl))
    return tmp1
...
def draw_bbox(tmp_dict, tmp_dict1):
    ...
    for dir_ in dirList:
        if (os.path.isdir(dir_)):
            ...
            for idx_im in tfile:
                ...
                if inDict_name in tmp_dict and inDict_name in tmp_dict1:
                    bboxList = tmp_dict[inDict_name]
                    gtList = tmp_dict1[inDict_name]
                    ...
                    # ax.imshow(im, aspect='equal')
                    if WRITE_TXT:
                        wtxt = open("/home/user/PycharmProjects/anaylsis_result/name_score.txt","a+")
                    for idx_bbox in bboxList:
                        for idx_gt in gtList:
                        #idx_bbox [0.999,375.544632086852,172.740631992242, 397.3884197828709,221.0229191797346]
                        #idx_gt [375.544632086852,172.740631992242,22.3884197828709,49.0229191797346]
                            idx_bbox_handle = str2float_in_list(idx_bbox[1:])
                            idx_gt_handle = str2float_in_list(idx_gt)
                            if len(idx_gt_handle) < 4 and  float(idx_bbox[0]) < 0.01:
                                continue
                            idx_gt_handle[2] += idx_gt_handle[0]
                            idx_gt_handle[3] += idx_gt_handle[1]
                            if DRAW_IOU and compute_IoU(idx_bbox_handle,idx_gt_handle):
                            ...
                            if DRAW_IMG and w >= 0 and h>=0:
                                ...
                                wtxt.write(str(inDict[-3] + '/' + inDict[-2]) + '/' + bot + ".jpg "+ str(idx_bbox))
                                wtxt.write("\n")
                                # plt.axis('off')
                            # plt.tight_layout()
                            # plt.imshow(im)
                            # plt.show()
                    mkdir('/home/user/det_result/py_Deformable_det_result/' + str(inDict[-3] + '/' + inDict[-2]))
                    if DRAW_IMG:
                        cv2.imwrite(
                                    '/home/user/det_result/py_Deformable_det_result/' + str(inDict[-3] + '/' + inDict[-2]) + '/' + bot + ".jpg",
                                    im)
                    elif WRITE_TXT:
                        wtxt.close()
...
DRAW_IOU = 0
WRITE_TXT = 0
file_dict = generate_all_result("/home/user/Downloads/caltech_data_set/data_reason_data_set1/")
generate_result("/home/user/PycharmProjects/caltech_new_anno/result/pyDeformable.txt", file_dict)
