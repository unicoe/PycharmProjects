...
for type in types:
	...
	res_path = os.path.join('/home/user/PycharmProjects/some_learn/Data_Set_handle/CityPersons-Dataset/ann_code_src/cache', type)
	image_data = []
	...
	for l in range(len(annos[index][0])):
					...
					for row_i in range(x2-x1, 1000, x2-x1+100):
						for col_j in range(y2-y1, 1000, (y2-y1+100)):
							...
							if flag < 9 and tmp_x1 >= 0 and tmp_x1 <= 2048 and tmp_x2 >= 0 and tmp_x2 <= 2048 and tmp_y1 >= 0 and tmp_y1 <= 1024 and tmp_y2 >= 0 and tmp_y2 <= 1024:
								...
								if  len(np.where(b1==True)[0]) <= 0 and len(np.where(b3==True)[0]) <=0 and len(np.where(b4==True)[0]) <= 0:
									(x1, y1, x2, y2) = boxes[i, :]
									if y2 - y1 > 50:
		...
		if(tmp_bboxes.size > 0):
			boxes = np.concatenate((boxes, tmp_bboxes))
				...
				# 					tmp_y2 = y2 + col_j
				#
				# 					if flag < 1 and tmp_x1 >= 0 and tmp_x1 <= 2048 and tmp_x2 >= 0 and tmp_x2 <= 2048 and tmp_y1 >= 0 and tmp_y1 <= 1024 and tmp_y2 >= 0 and tmp_y2 <= 1024:
				# 						c_tmp = np.array([[tmp_x1, tmp_y1, tmp_x2, tmp_y2]])
				# 						b1 = [[False]]
				# 						if (len(boxes) > 0) and len(c_tmp) > 0:
				# 							boxig_overlap1 = box_op(np.ascontiguousarray(boxes, dtype=np.float),
				...
				# 						b4 = [[False]]
				# 						if (len(tmp_bboxes) > 0):
				# 							boxig_overlap4 = box_op(
				# 								np.ascontiguousarray(np.array(tmp_bboxes), dtype=np.float),
				# 								np.ascontiguousarray(c_tmp, dtype=np.float))
				# 							b4 = boxig_overlap4 > 0.01
				#
				# 						# a=np.where(b4 == True)
				#
				# 						if len(np.where(b1 == True)[0]) <= 0 and len(
				# 								np.where(b3 == True)[0]) <= 0 and len(np.where(b4 == True)[0]) <= 0:
				# 							(x1, y1, x2, y2) = boxes[i, :]
				# 							if y2 - y1 > 50:
				# 								# crop_img = img[x1:x2, y1:y2]
				# 								crop_img = img[y1:y2, x1:x2]
		...
		# for i in range(len(tmp_bboxes)):
		# 	(x1, y1, x2, y2) = tmp_bboxes[i, :]
		# 	if y2 - y1 > 50:
		...
		mkdir("/home/user/Disk1.8T/data_set/city_new_data_1/" + i_folder + '/')

		#img.save("/home/user/Disk1.8T/data_set/city_new_data/" + i_folder + '/' + i_im + ".png", 'png')
		cv2.imwrite("/home/user/Disk1.8T/data_set/city_new_data_1/" + i_folder + '/' + i_im + ".png", img)
		# cv2.imshow("img", cv2.resize(img, (1024, 512)))
		#
		# print(img_path)
		# cv2.waitKey(0)
