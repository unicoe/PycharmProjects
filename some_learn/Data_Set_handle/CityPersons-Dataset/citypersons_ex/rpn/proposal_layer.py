...
'''
# todo 2018-10-17 15:47:03 当前只有一个rpn_rois层,目标:设置多个层,为不同的分支,分别分配rois
# 两个分支,做两个rois的输出,这两个分支,分别进行rois的处理
...
class ProposalLayer(caffe.Layer):
    ...
    """
    #通过估计边界框变换，应用于规则框（称为anchors），来输出预选框
    ...
    def forward(self, bottom, top):
        ...
        proposal_list = []
        score_list = []
        for s in self._feat_stride: #不同尺度的stride不同
            stride = int(s)
            ...
            # 3.按照阈值移除
            keep = _filter_boxes(proposals, min_size * im_info[2]) # 过滤长宽小于min_size的
            proposals = proposals[keep, :]
            scores = scores[keep]

            proposal_list.append(proposals) # 多分支 proposal_list
            score_list.append(scores) # 多分支 score_lisr


        #--2018-10-17 16:00:46 之前是将生成的两个结果合并到一起,现在不能做合并,要分开处理.


        # proposals = np.vstack(proposal_list) #np.vstack --> 垂直连接
        ...
        # 4. 形成(proposal, score)对，按照分数排序proposal
        # 5. 采用前pre_nms_topN个（做nms前）

        #--2018-10-17 16:05:36 从不同feature_map生成的proposal,进行分别处理,然后分两支传出
