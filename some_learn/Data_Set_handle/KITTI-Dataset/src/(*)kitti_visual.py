    ...
    for idx_im in dirList:
        ...
        if inDict_name in tmp_dict :
            ...
            if DRAW_IMG:
                im = cv2.imread(all_str)
                im = im[:, :, (2, 1, 0)]
                fig, ax = plt.subplots(figsize=(12.42, 3.75))
                ax.imshow(im, aspect='equal')
            ...
            mkdir('/home/user/Disk1.8T/draw_result/kitti/11_20_test/' + str(inDict[-3] + '/' + inDict[-2]))
            if DRAW_IMG:

                plt.axis('off')
                plt.tight_layout()
                plt.draw()
                plt.savefig('/home/user/Disk1.8T/draw_result/kitti/11_20_test/' + str(inDict[-3] + '/' + inDict[-2]) + '/' + bot + ".jpg")
                #plt.show()
...
file_path = "/home/user/PycharmProjects/some_learn/Data_Set_handle/KITTI-Dataset/det_result/det/2018_11_19_Mon_22_03_45_det_test_person.txt"
