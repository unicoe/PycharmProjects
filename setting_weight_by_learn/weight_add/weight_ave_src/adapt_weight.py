        ...
        for idx2 in range(len(np_info2)):
            ...
            if compute_IoU(tmp_bbox1.tolist(), tmp_bbox2.tolist()) > 0.4:
                if vis1[idx] != 1 and vis2[idx2] != 1:
                    res_bbox = tmp_bbox1 * pro1 + tmp_bbox2 * pro2 + b1
                    res_scores = tmp_score1 * pro1 + tmp_score2 * pro2 + b1
                    if res_scores[0] > 1.0:
                        res_scores[0] = 1.0
...
def write_result(res_list):
    wf = open("/home/user/PycharmProjects/handle_result/6_25(7_18)/weight_add/result/weight_handle_result0.576.txt", "w")
